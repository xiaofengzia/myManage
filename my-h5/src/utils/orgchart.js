export default class OrgChart {
  constructor(options) {
    ...
    chart.setAttribute('id', 'orgchart');
    chart.setAttribute('class', 'orgchart' + (opts.chartClass !== '' ? ' ' + opts.chartClass : '') +
      (opts.direction !== 't2b' ? ' ' + opts.direction : ''));

    // ADD BY XWY
    let exportBtn = document.getElementById('oc-export-btn');
    let downloadBtn = document.getElementById('oc-download-btn');
    if(downloadBtn && exportBtn) {
      let timestamp = (new Date()).valueOf();
      exportBtn.addEventListener('touchstart', this._clickExportButton.bind(this));
      downloadBtn.setAttribute('download', timestamp + '.png');
    }
    ...
    if (opts.zoom) {
      ...
      chartContainer.addEventListener('touchmove', this._onTouchMove.bind(this));
      chartContainer.addEventListener('touchend', this._onTouchEnd.bind(this));

    }

    if (opts.dbClick) {
      chartContainer.addEventListener('touchstart', this._dbclickHandler.bind(this));
    }
  ...
  _dbclickHandler(event) {
    event.stopPropagation();
		...
		let touchs = this._getTouches(event);
		if (touchs.length == 1) {
			if (!el.lastClickTime) {
				el.lastClickTime = time;
			}else{
				if (time - el.lastClickTime < 300) {
              this._createDetailInfoNode(event);
				}else{
					el.lastClickTime = time;
				}
			}
...
debugger
  ...
  _createNode(nodeData, level) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      nodeDiv.innerHTML = `
        <div class="title `+color+`">${nodeData[opts.nodeTitle]}
        ${nodeData.iconContent ? `<div id="subscript" class="subscript">${nodeData.iconContent}</div>` : ''}</div>
        ${opts.nodeContent ? `<div class="content `+color+`">${nodeData[opts.nodeContent]}</div>` : ''}
  ...
  buildHierarchy(appendTo, nodeData, level, callback) {
    ...
    if (Object.keys(nodeData).length > 1) { // if nodeData has nested structure
      ...
      isVerticalNode ? appendTo : nodeWrapper.setAttribute('id', 'table');
  ...
  _setChartScale(chart, newScale) {
    ...
    // chart.style.transformOrigin = 'left top';
    // chart.style.transition = 'all .3s ease-in-out 0s'
  ...
  _setTransform(chart, scale, needAnimation) { 
      ...
      chart.style.transformOrigin = 'left top';
      chart.style.transform = 'matrix('+scale+',0,0,'+scale+','+distanceX+','+distanceY+')';
  ...
  _touchZoom(flag) {
    if(flag) {
      this._setChartScale(this.chart, 1.2);
    }else{
      this._setChartScale(this.chart, 0.8);
    }
  ...
  _onTouchStart(event) {
    ...
    if (event.touches && event.touches.length === 2) {
      ...
      // console.log("_onTouchStart", dist)
  ...
  _onTouchMove(event) {
    ...
    // this._setChartScale(chart, 1.01);
    ...
    if (chart.dataset.pinching) {
      ...
      if (chart.dataset.pinching) {
        ...
        console.log(this.oldZoom, diff)
        
        
        if (diff > 0) {
          this._setChartScale(chart, 1.01);
        } else if (diff < 0) {
          this._setChartScale(chart, 0.99 );
        }
        this.oldZoom = diff;
      }

      // console.log("_onTouchMove", dist)
    }
  }
  _onTouchEnd(event) {
